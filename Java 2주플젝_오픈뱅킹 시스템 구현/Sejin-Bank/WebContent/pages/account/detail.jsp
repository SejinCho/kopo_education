...
<head>
	...
	<script type="text/javascript">
			...
			function getAccountInfo() {
				...
				$.ajax({
					...
					success : function(data) {
				        ...
				        document.getElementById('info_account_number').innerText = juMinWithHyphen($('#select_bank_name').val(), obj.account_number)
				        document.getElementById('info_balance').innerText = numberWithCommas(obj.balance)
		...
		function transferInfo() {
			...
			$.ajax({
				type: "POST",
				url : "<%=request.getContextPath()%>/account/transferInfoAjax.do",
				method : 'post',
				data : {
					"account_number" : $('#select_account_number').val(),
				...
				success : function(data) {
			        ...
			        for(let i = 0; i < transferList.length; ++i ) {
			        	...
			        	$('#transInfo > tbody').append('<tr>')
			        		...
			        		if(info.inout_type == 'O') { //출금
			        			$('#transInfo > tbody').append('<td>'+ numberWithCommas(info.tran_amt) +'</td>')
			        			$('#transInfo > tbody').append('<td></td>')
			        			if(info.content == null || info.content == ''){
			        				info.content = '출금'
			        			}
			        		} else if(info.inout_type == 'I') { //입금
			        			$('#transInfo > tbody').append('<td></td>')
			        			$('#transInfo > tbody').append('<td>'+ numberWithCommas(info.tran_amt) +'</td>')
				        			if(info.content == null || info.content == ''){
				        				info.content = '입금'
		...
		function juMinWithHyphen(bank_code, x) {
			if(bank_code == 'J') {
			    return x.toString().replace(/(\d{3})(\d{6})(\d{5})/, '$1-$2-$3');
				
			}else {
			    return x.toString().replace(/(\d{4})(\d{3})(\d{6})/, '$1-$2-$3');
			}
		}
