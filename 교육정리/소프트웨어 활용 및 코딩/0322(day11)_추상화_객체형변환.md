...
- **상속**
  ...
  - 'A는 B다'. '또는 A는 B의 한 종류다.' 라는 관계가 만족해야한다. 그러면 A를 상속받은 B를 만들 수 있다.
    - A를 부모 클래스, B는 자식 클래스
    - extends
  - 접근제한자
  ...
  - 어떤 클래스를 만들든지 내부적으로 Object class를 상속 받는다. 그래서 super()라는 키워드가 있는 것이다. 자바는 단일 상속만 지원하므로 import object를 사용하지 않는다.(사용하면 다른 class를 상속하지 못하므로)
